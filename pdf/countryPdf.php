...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_country_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(country, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(95);
        $this->Cell(20,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(country_name),1,0,'C',false);
...
if(count($countrys)!=0){
    ...
    foreach ($countrys as $k1 => $country) {
        $pdf->Row(array($country["idaddress"],
                $country["name"],
                ($country["status"])? activate : deactivate));
        $pdf->Cell(95);
    }
