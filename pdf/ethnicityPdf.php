...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_ethnicity_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(ethnicity, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(95);
        $this->Cell(20,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(ethnicity_name),1,0,'C',false);
...
$pdf->SetWidths(array(20,50,26));
$pdf->SetAligns(array("C","C","C"));
if(count($ethnicitys)!=0){
    $pdf->SetFillColor(0,0,140); 
    $pdf->SetFont('Arial','',8);
    $pdf->SetTextColor(0,0,0); 
    foreach ($ethnicitys as $k1 => $ethnicity) {
        $pdf->Row(array($ethnicity["idethnicity"],
                $ethnicity["name"],
                ($ethnicity["status"])? activate : deactivate));
        $pdf->Cell(95);
    }
