...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_log_access_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(log_access, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(25);
        $this->Cell(15,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(26,7,utf8_decode(log_access_name),1,0,'C',false);
        $this->Cell(26,7,utf8_decode(log_access_message),1,0,'C',false);
        $this->Cell(35,7,utf8_decode(log_access_ip),1,0,'C',false);
        $this->Cell(38,7,utf8_decode(log_access_browser),1,0,'C',false);
        $this->Cell(45,7,utf8_decode(log_access_date_created),1,0,'C',false);
        $this->Cell(45,7,utf8_decode(log_access_operative_system),1,1,'C',false);
        $this->Cell(25);
    }
}
$pdf=new clsFpdf;
...
$pdf->SetWidths(array(15,26,26,35,38,45,45));
$pdf->SetAligns(array("C","C","C","C","C","C","C"));
if(count($log_accesss)!=0){
    $pdf->SetFillColor(0,0,140); 
    $pdf->SetFont('Arial','',8);
    $pdf->SetTextColor(0,0,0); 
    foreach ($log_accesss as $k1 => $log_access) {
        $pdf->Row(array($log_access["idlog_access"],
                $log_access["name"],
                $log_access["message"],
                $log_access["ip"],
                $log_access["browser"],
                $log_access["date_created"],
                $log_access["operative_system"]));
        $pdf->Cell(25);
    }
