...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_log_movement_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(log_movement, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(5);
        $this->Cell(15,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(26,7,utf8_decode(log_movement_user),1,0,'C',false);
        $this->Cell(20,7,utf8_decode(log_movement_action),1,0,'C',false);
        $this->Cell(30,7,utf8_decode(log_movement_service),1,0,'C',false);
        $this->Cell(30,7,utf8_decode(log_movement_message),1,0,'C',false);
        $this->Cell(110,7,utf8_decode(log_movement_data),1,0,'C',false);
        $this->Cell(35,7,utf8_decode(log_movement_date_created),1,1,'C',false);
        $this->Cell(5);
    }
}
$pdf=new clsFpdf;
...
$pdf->SetWidths(array(15,26,20,30,30,110,35));
$pdf->SetAligns(array("C","C","C","C","C","C","C"));
if(count($log_movements)!=0){
    $pdf->SetFillColor(0,0,140); 
    $pdf->SetFont('Arial','',8);
    $pdf->SetTextColor(0,0,0); 
    foreach ($log_movements as $k1 => $log_movement) {
        $pdf->Row(array(utf8_decode($log_movement["idlog_movement"]),
                utf8_decode($log_movement["uname"]),
                utf8_decode($log_movement["aname"]),
                utf8_decode($log_movement["sname"]),
                utf8_decode($log_movement["message"]),
                utf8_decode($log_movement["data"]),
                utf8_decode($log_movement["date_created"])));
        $pdf->Cell(5);
    }
