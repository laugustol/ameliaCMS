...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_log_report_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(log_report, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(45);
        $this->Cell(15,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(26,7,utf8_decode(log_report_user),1,0,'C',false);
        $this->Cell(40,7,utf8_decode(log_report_report),1,0,'C',false);
        $this->Cell(70,7,utf8_decode(log_report_code),1,0,'C',false);
        $this->Cell(38,7,utf8_decode(log_report_date_created),1,1,'C',false);
        $this->Cell(45);
    }
}
$pdf=new clsFpdf;
...
$pdf->SetWidths(array(15,26,40,70,38));
$pdf->SetAligns(array("C","C","C","C","C"));
if(count($log_reports)!=0){
    $pdf->SetFillColor(0,0,140); 
    $pdf->SetFont('Arial','',8);
    $pdf->SetTextColor(0,0,0); 
    foreach ($log_reports as $k1 => $log_report) {
        $pdf->Row(array($log_report["idlog_report"],
                $log_report["name"],
                $log_report["report"],
                $log_report["code"],
                $log_report["date_created"]));
        $pdf->Cell(45);
    }
