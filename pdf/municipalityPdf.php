...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_municipality_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(municipality, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(70);
        $this->Cell(20,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(address_name),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(municipality_father),1,0,'C',false);
...
if(count($municipalitys)!=0){
    $pdf->SetFillColor(0,0,140); 
    $pdf->SetFont('Arial','',8);
    $pdf->SetTextColor(0,0,0); 
    foreach ($municipalitys as $k1 => $municipality) {
        $pdf->Row(array($municipality["idaddress"],
                $municipality["name"],
                $municipality["father"],
                ($municipality["status"])? activate : deactivate));
        $pdf->Cell(70);
    }
