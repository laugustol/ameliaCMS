...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_nationality_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(nationality, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(70);
        $this->Cell(20,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(nationality_name_one),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(nationality_name_two),1,0,'C',false);
...
$pdf->SetWidths(array(20,50,50,26));
$pdf->SetAligns(array("C","C","C","C"));
if(count($nationalitys)!=0){
    $pdf->SetFillColor(0,0,140); 
    $pdf->SetFont('Arial','',8);
    $pdf->SetTextColor(0,0,0); 
    foreach ($nationalitys as $k1 => $nationality) {
        $pdf->Row(array($nationality["idnationality"],
                $nationality["name_one"],
                $nationality["name_two"],
                ($nationality["status"])? activate : deactivate));
        $pdf->Cell(70);
    }
