...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_parish_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(parish, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(70);
        $this->Cell(20,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(address_name),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(parish_father),1,0,'C',false);
...
if(count($parishs)!=0){
    ...
    foreach ($parishs as $k1 => $parish) {
        $pdf->Row(array($parish["idaddress"],
                $parish["name"],
                $parish["father"],
                ($parish["status"])? activate : deactivate));
        $pdf->Cell(70);
    }
