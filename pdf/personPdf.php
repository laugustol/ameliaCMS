...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_person_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(person, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(5);
        $this->Cell(15,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(26,7,utf8_decode(person_idnationality),1,0,'C',false);
        $this->Cell(26,7,utf8_decode(person_identification_card),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(names),1,0,'C',false);
        $this->Cell(38,7,utf8_decode(person_birth_date),1,0,'C',false);
        $this->Cell(13,7,utf8_decode(person_sex),1,0,'C',false);
        $this->Cell(45,7,utf8_decode(person_idaddress),1,0,'C',false);
        $this->Cell(26,7,utf8_decode(person_charge),1,0,'C',false);
...
$pdf->SetWidths(array(15,26,26,50,38,13,45,26,26));
$pdf->SetAligns(array("C","C","C","C","C","C","C","C","C"));
if(count($persons)!=0){
    $pdf->SetFillColor(0,0,140); 
    $pdf->SetFont('Arial','',8);
    $pdf->SetTextColor(0,0,0); 
    foreach ($persons as $k1 => $person) {
        $pdf->Row(array($person["idperson"],
                $person["name_one"],
                $person["identification_card"],
                $person["name"],
                $person["birth_date"],
                $person["sex"],
                $person["address"],
                $person["cname"],
                ($person["status"])? activate : deactivate));
        $pdf->Cell(5);
    }
