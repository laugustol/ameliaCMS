...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_post_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(post, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(70);
        $this->Cell(15,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(post_name),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(post_created_by),1,0,'C',false);
...
$pdf->SetWidths(array(15,50,50,26));
...
if(count($posts)!=0){
    ...
    foreach ($posts as $k1 => $post) {
        $pdf->Row(array($post["idpost"],
                $post["name"],
                $post["person"],
                ($post["status"])? activate : deactivate));
        $pdf->Cell(70);
    }
