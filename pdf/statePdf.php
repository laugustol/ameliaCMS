...
class clsFpdf extends FPDF{
    ...
    public function Header(){
        ...
        $this->Cell(0,6,utf8_decode(pdf_state_list),0,1,"C");
        $this->Ln(8);
        $this->SetFillColor(0,0,140);
        $this->SetTitle(state, true);
        $this->SetFont('Arial','B',10);
        $this->SetTextColor(0,0,0);
        $this->Cell(70);
        $this->Cell(20,7,utf8_decode(id),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(address_name),1,0,'C',false);
        $this->Cell(50,7,utf8_decode(state_father),1,0,'C',false);
...
if(count($states)!=0){
    ...
    foreach ($states as $k1 => $state) {
        $pdf->Row(array($state["idaddress"],
                $state["name"],
                $state["father"],
                ($state["status"])? activate : deactivate));
        $pdf->Cell(70);
    }
