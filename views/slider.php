...
<div class="box">
	<div class="box-tools">
		<div class="box-tool-left">
			<a href="<?=url_base?>home/dashboard"><?=dashboard?></a> <i class="fa fa-angle-right"></i> <a href="<?=url_base.routerCtrl?>"><?=slider?></a> <?=(action!="index")? "<i class='fa fa-angle-right'></i> ".((action=="add")? add : ((action=="edit")? edit : query ) ) : ''?>
		</div>
			...
			<table id="datatable" class="table table-striped table-bordered table-hover dataTable" width="100%">
                ...
                <?php
                	foreach($dependencies["list"] as $val){
                		echo "<tr><td>".$val["idslider"]."</td><td>".$val["name"]."</td><td>".$val["btn"]."</td></tr>";
                	}
                ?>
                <tfoot><th><?=id?></th><th><?=slider_name?></th><th><?=actions?></th></tfoot>
            </table>
		<?php }else{ ?>
			<?=(action!="query")? "<form action='".url_base.routerCtrl."/".action."/".$d[0]["idslider"]."' method='POST' class='form-horizontal'>" : "<div class='form-horizontal'>" ?>
